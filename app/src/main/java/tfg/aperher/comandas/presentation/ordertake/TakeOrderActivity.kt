...
import tfg.aperher.comandas.presentation.ordertake.articledetails.ArticleDetailsFragment.ArticleAddedListener
...
class TakeOrderActivity : AppCompatActivity(), MenuProvider, ArticleAddedListener {
    companion object {
        ...
        const val ORDER_ACTIVE = "ORDER_ACTIVE"
    ...
    private fun getExtrasArguments() {
        val sectionName = intent.extras?.getString(SECTION)!!
        val table = intent.extras?.getParcelable<Table>(TABLE)!!
        val isOrderActive = intent.extras?.getBoolean(ORDER_ACTIVE)!!
        viewModel.initOrder(sectionName, table, isOrderActive)
    }
